...
import static org.apache.flume.ext.KafkaChannelConfiguration.*;
...
public class KafkaChannel extends BasicChannelSemantics {
  ...
  // default use source mode
  private boolean useSinkMode = false;

  private boolean setTopicHeader;
  private String topicHeaderKey = null;

  private boolean setPartitionHeader;
  private String partitionHeaderKey = null;

  private boolean setOffsetHeader;
  private String offsetHeaderKey = null;
  ...
  public void configure(Context ctx) {
    ...
    useSinkMode = ctx.getBoolean(USE_SINK_MODE, false);
    logger.info("channel use sink mode {}", useSinkMode);
    ...
    if (useSinkMode) {
      ...
      topicHeaderKey = ctx.getString(TOPIC_HEADER_KEY, DEFAULT_TOPIC_HEADER);
      ...
      setTopicHeader = ctx.getBoolean(SET_TOPIC_HEADER, DEFAULT_SET_HEADER);
      topicHeaderKey = ctx.getString(TOPIC_HEADER_KEY, DEFAULT_TOPIC_HEADER);

      setPartitionHeader = ctx.getBoolean(SET_PARTITION_HEADER, DEFAULT_SET_HEADER);
      partitionHeaderKey = ctx.getString(PARTITION_HEADER_KEY, DEFAULT_PARTITION_HEADER);

      setOffsetHeader = ctx.getBoolean(SET_OFFSET_HEADER, DEFAULT_SET_HEADER);
      offsetHeaderKey = ctx.getString(OFFSET_HEADER_KEY, DEFAULT_OFFSET_HEADER);
    }
  ...
  private synchronized ConsumerAndRecords createConsumerAndRecords() {
    ...
    try {
      ...
      car.consumer.subscribe(Pattern.compile(topic.get()),
  ...
  private class KafkaTransaction extends BasicTransactionSemantics {
    ...
    protected void doPut(Event event) throws InterruptedException {
      ...
      // producer topic used by topic header
      String producerTopic = event.getHeaders().get(topicHeaderKey);
      if (StringUtils.isBlank(producerTopic)) {
        producerTopic = topic.get();
      }
      ...
      try {
        ...
        if (partitionId != null) {
          producerRecords.get().add(
              new ProducerRecord<String, byte[]>(producerTopic, partitionId, key,
                  serializeValue(event, parseAsFlumeEvent)));
        } else {
          producerRecords.get().add(
              new ProducerRecord<String, byte[]>(producerTopic, key,
                  serializeValue(event, parseAsFlumeEvent)));
        }
      ...
      if (!consumerAndRecords.get().failedEvents.isEmpty()) {
        ...
        try {
          ...
          if (consumerAndRecords.get().recordIterator.hasNext()) {
            ...
            if (setTopicHeader) {
              e.getHeaders().put(topicHeaderKey, tp.topic());
            }

            // set the partition header
            if (setPartitionHeader) {
              e.getHeaders().put(partitionHeaderKey, String.valueOf(tp.partition()));
            }

            // set the offset header
            if (setOffsetHeader) {
              e.getHeaders().put(offsetHeaderKey, String.valueOf(record.offset()));
            }
